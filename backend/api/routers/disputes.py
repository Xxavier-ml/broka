...
from api.database import (
    get_db, Deal, Dispute, DisputeStatus, User, Listing,
...
async def _call_ai(system: str, messages: List[dict]) -> str:
    if GEMINI_API_KEY:
        try:
            ...
            logger.warning("Gemini failed for dispute (%s) - trying Groq", e)
    if GROQ_API_KEY:
        try:
    ...
    raise HTTPException(status_code=503, detail="AI not configured - set GEMINI_API_KEY or GROQ_API_KEY")
...
async def _mpesa_b2c_refund(phone: str, amount: float, dispute_id: str) -> dict:
    """
    ...
    payload = {
        ...
        "PartyA":             SHORTCODE,
        "PartyB":             p,
        "Remarks":            f"BROKA dispute refund {dispute_id[:8].upper()}",
        "QueueTimeOutURL":    B2C_TIMEOUT_URL,
        "ResultURL":          B2C_RESULT_URL,
        "Occasion":           f"BROKA-DISPUTE-{dispute_id[:8].upper()}",
    }
    ...
    if resp.status_code == 200 and body.get("ResponseCode") == "0":
        logger.info("[disputes] B2C refund initiated for dispute %s → phone %s", dispute_id, p)
...
async def mediate(
    ...
    logger.info(
        ...
        "gemini/groq" if (GEMINI_API_KEY or GROQ_API_KEY) else "rule-based"
    )
